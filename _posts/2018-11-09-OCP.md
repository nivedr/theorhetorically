...
2. The update rule of the algorithm generates an upper-bound on this linear function comprising of two components - a difference of potentials that captures the $\ell_2$-convergence of $$\{ \dots,x_t,x_{t+1},\dots\}$$ to $$x^\*$$ and a second-order error term due to the update rule moving along the local gradient $$(\nabla g_t )(x_t)$$ and not the global gradient $$(\nabla \sum_t g_t) (x_t)$$.
